// Most of this file and the src/utils folders are direct TypeScript translations from
// The BootstrapVue project. Minor modifications made as required.

// Some shims to make code compile
...
const moduleExports = require(path.resolve(baseDir, "dist/index.es.js"));
...
const computePropType = ({ type }) => {
  ...
  return ["Boolean", "String", "Number", "Function", "Object"].indexOf(type) >
    -1
    ? type.toLowerCase()
    : type;
};
...
const computePropDefault = ({ default: def, type }) => {
  ...
  //   try {
  //     def = def.call({}); // there's async code here not being waited on
  //   } catch (e) {
  //     // Well, that's interesting. Might be that invalid arguments were given to a real function
  //   }
  // }
  if (
    type === Boolean ||
    (Array.isArray(type) && type[0] === Boolean && !def)
  ) {
    def = Boolean(def);
    ...
    tag.attributes = Object.keys($props).map((propName) => {
      ...
      const type =
        typeof $prop === "function"
          ? computePropType({ type: $prop })
          : computePropType($prop);
      const prop = {
        name: propName,
        value: {
          kind: "expression",
  ...
  if ($aliases.length) {
    ...
    $aliases.forEach((alias) => {
      const aliasTag = {
        ...tag,
        name: alias,
        source: { ...tag.source, symbol: alias },
      };
...
const processComponentGroup = (groupSlug) => {
  ...
  componentsMeta.forEach((meta) => {
    processComponentMeta(meta, moduleExports, groupMeta.description, docUrl);
  });
  ...
  Object.keys(webTypes.contributions.html.tags).forEach((component) => {
    ...
    def.attributes.forEach((attrObj) => {
      ...
      veturAttributes[`${tag}/${kebabCase(attrObj.name)}`] = {
        description:
          attrObj.description || `One of: ${type.split("|").join(" or ")}`,
        type: type,
      };
    });
  });
